...
import { rowItemsRenderer, rowLayerRenderer, getNearestRowNumber, getNearestRowObject, getMaxOverlappingItems, getTrueBottom, getVerticalMarginBorder, getRowObjectRowNumber } from "./utils/itemUtils"
...
import "core-js/fn/string/starts-with" // startsWith polyfill for IE11 support
...
export default class Timeline extends React.Component {
    ...
    if (canDrag) {
      this._itemInteractable
        ...
        .on("dragmove", (e) => {
          ...
          const snapDx = getSnapPixelFromDelta(dx, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
          ...
          _.forEach(animatedItems, (domItem) => {
            ...
            let newStart = getTimeAtPixel(newPixelOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
        ...
        .on("dragend", (e) => {
          ...
          animatedItems.forEach((domItem) => {
            ...
            domItem.style["top"] = intToPix(this.props.itemHeight * Math.round(pixToInt(domItem.style["top"]) / this.props.itemHeight))
    ...
    if (canResize) {
      this._itemInteractable
        ...
        .on("resizemove", (e) => {
          ...
          const minimumWidth = pixelsPerMinute(this.props.startDate, this.props.endDate, this.getTimelineWidth()) * this.props.snapMinutes

          const snappedDx = getSnapPixelFromDelta(dx, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)

          const snappedDw = getSnapPixelFromDelta(dw, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
        ...
        .on("resizeend", (e) => {
          ...
          _.forEach(animatedItems, (domItem) => {
            ...
            if (isStartTimeChange) {
              let newStart = getTimeAtPixel(startPixelOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
              ...
              let newEnd = getTimeAtPixel(endPixelOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
      ...
      this._selectRectangleInteractable
        ...
        .on("dragend", (e) => {
          ...
          if (topRowObject !== undefined) {
            ...
            const bottomRow = Number(getNearestRowNumber(left + width, Math.floor(topRowLoc.top - rowMarginBorder) + Math.floor(height - rowMarginBorder)))
            ...
            const startTime = getTimeAtPixel(startOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
            const endTime = getTimeAtPixel(endOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth(), this.props.snapMinutes)
  ...
  _handleItemRowEvent = (e, itemCallback, rowCallback) => {
    ...
    if (e.target.hasAttribute("data-item-index") || e.target.parentElement.hasAttribute("data-item-index")) {
      ...
      let clickedTime = getTimeAtPixel(e.clientX - this.props.groupOffset, this.props.startDate, this.props.endDate, this.getTimelineWidth())
    ...
    return ({ columnIndex, key, parent, rowIndex, style }) => {
      ...
      if (itemCol === columnIndex) {
        ...
        return (
          ...
          >
            {rowItemsRenderer(itemsInRow, this.props.startDate, this.props.endDate, width, this.props.itemHeight, this.props.itemRenderer, canSelect ? this.props.selectedItems : [])}
  ...
  getCursor() {
    ...
    if (!cursorTime) {
      // cursorTime = moment.utc('2017-05-17T12:59:59Z')
    }

    // console.log(cursorTime.utc().clone().format(cursorTimeFormat));
    return showCursorTime && cursorTime ? cursorTime.utc().clone().format(cursorTimeFormat) : null
  }
        ...
        <AutoSizer className="rct9k-autosizer" onResize={this.refreshGrid}>
          {({ height, width }) => (
            <div className="parent-div" onMouseMove={this.mouseMoveFunc}>
              ...
              <Timebar cursorTime={this.getCursor()} start={this.props.startDate} end={this.props.endDate} width={width} leftOffset={groupOffset} selectedRanges={this.state.selection} groupTitleRenderer={groupTitleRenderer} {...varTimebarProps} />
              {markers.map((m) => (
