...
export function rowItemsRenderer(items, vis_start, vis_end, total_width, itemHeight, itemRenderer, selectedItems = []) {
  ...
  return _.map(displayItems, (i) => {
    ...
    if (width < 20) {
      outerClassnames += " small-width-item"
    }
    return (
      <span key={i.key} data-item-index={i.key} className={outerClassnames} style={{ left, width, top, backgroundColor: "transparent" }}>
...
export function rowLayerRenderer(layers, vis_start, vis_end, total_width, itemHeight) {
  ...
  return _.map(displayItems, (i) => {
    ...
    return <div key={`r-${rowNumber}-${i.start.unix()}`} data-item-index={i.key} className={outerClassnames} style={{ ...style, left, width, top, height }} />
  })
}
...
export function getVerticalMarginBorder(elem) {
  ...
  const rowMargins = (Math.ceil(parseFloat(computedStyles["marginTop"]) + parseFloat(computedStyles["marginBottom"])) || 1) / 2
  // half the size of the border seems important
  const rowBorders = (Math.ceil(parseFloat(computedStyles["borderTopWidth"]) + parseFloat(computedStyles["borderBottomWidth"])) || 1) / 2
